...
function objetRegrp(data) {
    const regrp = {};
    for (let i=0; i<data.length; i++) {
        const groupe = data[i].groupe;
        const nom = data[i].nom;
        console.log(groupe, nom);
    if (!regrp[groupe]) {
      regrp[groupe] = [];
    }
    regrp[groupe].push({ nom: nom });
    } 
    return regrp;
}
let data = [{nom: "A", groupe: 1}, {nom: "B", groupe: 2}, {nom: "C", groupe: 1}];
console.log(objetRegrp(data));
